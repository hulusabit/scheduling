...
public class Individual {
	...
	public Individual(Schedule schedule) {
		int numMatches = schedule.getNumMatches();

		// 1 gene for awayteam, 1 for time, 1 for professor
		int chromosomeLength = numMatches * 3;
		// Create random individual
		int newChromosome[] = new int[chromosomeLength];
		int chromosomeIndex = 0;
		// Loop through groups
		for (HomeTeams hometeams : schedule.getHomeTeamsAsArray()){
		
				
				// Add random referee
				int refereeId = schedule.getRandomReferee().getRefereeId();
				newChromosome[chromosomeIndex] = refereeId;
	
				chromosomeIndex++;
				
				
				// Add random time
				int awayteamId = schedule.getRandomAwayTeams().getAwayteamId();
				newChromosome[chromosomeIndex] = awayteamId;
				chromosomeIndex++;
